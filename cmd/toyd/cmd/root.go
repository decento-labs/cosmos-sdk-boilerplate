...
func NewRootCmd() (*cobra.Command, EncodingConfig) {
	...
	encodingConfig := MakeEncodingConfig(chain.ModuleBasics)
...
func initRootCmd(rootCmd *cobra.Command, encodingConfig EncodingConfig) {
	...
	// add keybase, auxiliary RPC, genesis, query, and tx child commands
	rootCmd.AddCommand(
		rpc.StatusCommand(),
...
func genesisCommand(encodingConfig EncodingConfig, cmds ...*cobra.Command) *cobra.Command {
	cmd := genutilcli.GenesisCoreCommand(encodingConfig.TxConfig, chain.ModuleBasics, chain.DefaultNodeHome)
...
func appExport(
	...
	if height != -1 {
		app = chain.NewApp(logger, db, traceStore, false, appOpts)

		if err := app.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		app = chain.NewApp(logger, db, traceStore, true, appOpts)
	}
